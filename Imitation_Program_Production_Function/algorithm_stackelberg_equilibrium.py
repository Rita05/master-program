...
def find_stackelberg_equilibrium(bank: CommercialBank, entrepreneur: Entrepreneur):
    ...
    # bank.display_info_()
    # samples1, step1 = np.linspace(PR_MIN, PR_MAX, 10, retstep=True, endpoint=True)
    # samples2, step2 = np.linspace(M_MIN, M_MAX, 10, retstep=True, endpoint=True)
    ...
    for pr in np.linspace(PR_MIN, PR_MAX, 10, endpoint=True):
        ...
        #and entrepreneur.profite_sale_goods(pr, m) - \
            #entrepreneur.sum_credit_bank(pr, m) <= 4500000
